        ...
        match self {
            BencodeValue::String(bytes) => {
                String::from_utf8(bytes.clone())
                    .expect("BencodeValue string is not valid UTF-8")
            }
        ...
        match self {
            BencodeValue::Integer(bytes) => {
                let s = std::str::from_utf8(bytes)
                    .expect("invalid UTF-8 inside Bencode integer");
                s.parse::<i64>()
...
fn parse_string(input_slice : &[u8]) -> (BencodeValue, usize)  {
    ...
    let string_size = std::str::from_utf8(&input_slice[..pos_end_string])
        ...
        .unwrap_or(0);
    (BencodeValue::String(input_slice[pos_end_string +1 ..pos_end_string + 1 + string_size].to_vec()), pos_end_string + string_size +1)
}
...
fn parse_integer(input_slice : &[u8]) -> (BencodeValue, usize) {
    ...
    (BencodeValue::Integer(input_slice[1..end_of_integer].to_vec()), end_of_integer +1 )
}
...
mod tests {
    ...
    fn bencode_parse_peers() {
        ...
        ip_map1.insert("ip".as_bytes().to_vec(), BencodeValue::Integer("46.5.64.254".as_bytes().to_vec()));
        ip_map1.insert("port".as_bytes().to_vec(), BencodeValue::Integer("6881".as_bytes().to_vec()));
        result_map.insert("interval".as_bytes().to_vec(),BencodeValue::Integer("900".as_bytes().to_vec()));
        result_map.insert("peers".as_bytes().to_vec(),BencodeValue::List(vec![BencodeValue::Dictionary(ip_map1)]));
